...
function doQuery($index) {
	...
	$sql = array(
		"SELECT MIN(Temp_C), MAX(Temp_C), AVG(Temp_C) FROM SensorLogs WHERE SensorID=1 AND Date > SUBDATE(NOW(), INTERVAL 7 DAY)",
		"SELECT MIN(Temp_C), MAX(Temp_C), AVG(Temp_C) FROM SensorLogs WHERE SensorID=2 AND Date > SUBDATE(NOW(), INTERVAL 7 DAY)",
		"SELECT MIN(Temp_C), MAX(Temp_C), AVG(Temp_C) FROM SensorLogs WHERE SensorID=1 AND DATE(Date) = CURDATE()",
		"SELECT MIN(Temp_C), MAX(Temp_C), AVG(Temp_C) FROM SensorLogs WHERE SensorID=2 AND DATE(Date) = CURDATE()",
...
doQuery(0);
$inne['min_week'] = $min; $inne['max_week'] = $max; $inne['avg_week'] = $avg;

doQuery(1);
$ute['min_week'] = $min; $ute['max_week'] = $max; $ute['avg_week'] = $avg;


doQuery(2);
$inne['min_day'] = $min; $inne['max_day'] = $max; $inne['avg_day'] = $avg;

doQuery(3);
$ute['min_day'] = $min; $ute['max_day'] = $max; $ute['avg_day'] = $avg;
...
<div id="ute">
 ...
 <table>
 <tr>
 <td colspan="2" class="rubrik">Senaste 7 dagarna</td></tr>
